...
PREVIEW_DIR="$(mktemp -d "${TMPDIR:-/tmp}/kas-studio-preview.XXXXXX")"
...
cleanup() {
  ...
  if [[ "$PREVIEW_DIR" == "${TMPDIR:-/tmp}"/kas-studio-preview.* ]]; then
    rm -rf "$PREVIEW_DIR"
  fi
  exit "$status"
}
...
cd "$ROOT"
echo "Building KAS and Studio packages..."
cargo build --workspace
"$STUDIO_ROOT/scripts/build-packages.sh" "$PACKAGES_DIR"
...
echo
echo "KAS Studio preview is ready"

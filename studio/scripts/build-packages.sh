...
OUTPUT_DIR="${1:-$STUDIO_ROOT/dist}"
PROFILE="${KAS_STUDIO_PROFILE:-debug}"
TARGET_DIR="${CARGO_TARGET_DIR:-$STUDIO_ROOT/target}"
STAGING_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/kas-studio-packages.XXXXXX")"

cleanup() {
  if [[ "$STAGING_ROOT" == "${TMPDIR:-/tmp}"/kas-studio-packages.* ]]; then
    rm -rf "$STAGING_ROOT"
  fi
}
trap cleanup EXIT

if [[ ! -d "$STUDIO_ROOT/frontend/node_modules" ]]; then
  npm --prefix "$STUDIO_ROOT/frontend" ci
fi
npm --prefix "$STUDIO_ROOT/frontend" run build

case "$PROFILE" in
  debug)
    cargo build \
      --manifest-path "$STUDIO_ROOT/Cargo.toml" \
      -p kas-agent-driver \
  ...
  release)
    ...
    ;;
  *)
    echo "KAS_STUDIO_PROFILE must be debug or release" >&2
    exit 2
    ;;
esac
...
cp "$STUDIO_ROOT/packages/agent/manifest.json" "$STAGING_ROOT/agent/manifest.json"
cp -R "$STUDIO_ROOT/packages/agent/resources" "$STAGING_ROOT/agent/resources"
cp \
...
cp "$STUDIO_ROOT/packages/approval/manifest.json" "$STAGING_ROOT/approval/manifest.json"
cp -R "$STUDIO_ROOT/packages/approval/resources" "$STAGING_ROOT/approval/resources"
cp \
...
cp "$STUDIO_ROOT/packages/approval-result/manifest.json" "$STAGING_ROOT/approval-result/manifest.json"
cp "$STUDIO_ROOT/packages/file/manifest.json" "$STAGING_ROOT/file/manifest.json"
cp -R "$STUDIO_ROOT/packages/file/resources" "$STAGING_ROOT/file/resources"
cp \
...
cp "$STUDIO_ROOT/packages/frontend/manifest.json" "$STAGING_ROOT/frontend/manifest.json"
cp -R "$STUDIO_ROOT/packages/frontend/resources" "$STAGING_ROOT/frontend/resources"
cp \
...
cp -R "$STUDIO_ROOT/frontend/dist/." "$STAGING_ROOT/frontend/driver/web/"
cp "$STUDIO_ROOT/packages/message/manifest.json" "$STAGING_ROOT/message/manifest.json"
cp -R "$STUDIO_ROOT/packages/message/resources" "$STAGING_ROOT/message/resources"
cp \
...
cp "$STUDIO_ROOT/packages/proxy/manifest.json" "$STAGING_ROOT/proxy/manifest.json"
cp "$STUDIO_ROOT/packages/skill/manifest.json" "$STAGING_ROOT/skill/manifest.json"
cp -R "$STUDIO_ROOT/packages/skill/resources" "$STAGING_ROOT/skill/resources"
cp \
...
cp "$STUDIO_ROOT/packages/thread/manifest.json" "$STAGING_ROOT/thread/manifest.json"
cp -R "$STUDIO_ROOT/packages/thread/resources" "$STAGING_ROOT/thread/resources"
cp "$STUDIO_ROOT/packages/telegram/manifest.json" "$STAGING_ROOT/telegram/manifest.json"
cp -R "$STUDIO_ROOT/packages/telegram/resources" "$STAGING_ROOT/telegram/resources"
cp \
...
cp "$STUDIO_ROOT/packages/session/manifest.json" "$STAGING_ROOT/session/manifest.json"
cp -R "$STUDIO_ROOT/packages/session/resources" "$STAGING_ROOT/session/resources"

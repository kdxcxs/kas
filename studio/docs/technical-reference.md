# KAS Studio technical reference

This document covers the Studio Package layout, Agent and Session behavior,
File/Skill/Approval APIs, frontend plugin runtime, end-to-end tests, and Docker
deployment. Start with the [Studio overview](../README.md) if this is your
...
```bash
studio/scripts/build-packages.sh
```

This writes `studio/dist/thread.kas`, `studio/dist/session.kas`,
`studio/dist/file.kas`, `studio/dist/skill.kas`,
`studio/dist/message.kas`, `studio/dist/approval-result.kas`,
`studio/dist/approval.kas`, and `studio/dist/agent.kas`. Install Approval
...
Result Resources after removing Core `[kas]` bookkeeping fields and
are discovered through `result-of` and `produced-by` Links.

Run the complete Studio flow with the real Codex CLI:

```bash
studio/tests/e2e.sh
studio/tests/e2e.sh -v
```
...
Driver RBAC, the HTTP Gateway, and the plugin runtime. Build all Studio
packages with:
...
Studio extensions can contribute entries to the Workspace or Resources
...
```bash
studio/scripts/build-frontend-plugin.sh \
  studio/plugins/registry \
  /tmp/registry.zip
...
`studio/plugins/registry/` implements the generic Objects registry.
...
Build the complete Studio image from the repository root:

```bash
docker build -f studio/Dockerfile -t kas-studio .
```
...
The multi-stage build compiles Core and every Studio Driver in release mode,

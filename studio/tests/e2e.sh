...
E2E_DIR="$(mktemp -d "${TMPDIR:-/tmp}/kas-studio-e2e.XXXXXX")"
CORE_TARGET="${KAS_CORE_TARGET_DIR:-$E2E_DIR/core-target}"
STUDIO_TARGET="${KAS_STUDIO_TARGET_DIR:-$E2E_DIR/studio-target}"
API_PID=""
TELEGRAM_MOCK_PID=""
...
cleanup() {
  ...
  if [[ "${KAS_E2E_KEEP:-false}" != true ]] &&
    [[ "$E2E_DIR" == "${TMPDIR:-/tmp}"/kas-studio-e2e.* ]]; then
    rm -rf "$E2E_DIR"
  fi
}

failed() {
  echo "Studio E2E failed at line $1" >&2
...
CARGO_TARGET_DIR="$STUDIO_TARGET" "$STUDIO_ROOT/scripts/build-packages.sh" "$E2E_DIR/packages"
...
ADMIN_TOKEN="$("$CORE_TARGET/debug/kas-admin" bootstrap studio-admin)"
...
python3 "$STUDIO_ROOT/tests/mock_telegram.py" \
...
"$STUDIO_ROOT/scripts/build-workspace-plugin.sh" \
  "$STUDIO_ROOT/frontend/dist" \
...
(cd "$STUDIO_ROOT/tests/fixtures/skills/e2e-v1" && zip -qr "$SKILL_V1_BUNDLE" .)
(cd "$STUDIO_ROOT/tests/fixtures/skills/e2e-v2" && zip -qr "$SKILL_V2_BUNDLE" .)

INVALID_SKILL_DIR="$E2E_DIR/invalid-skill"
cp -R "$STUDIO_ROOT/tests/fixtures/skills/e2e-v1" "$INVALID_SKILL_DIR"
